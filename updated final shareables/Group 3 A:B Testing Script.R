...
### Load in Data
final_data1 <- read.csv("Group 3 Clean_data.csv", stringsAsFactors = FALSE)
...
# The previous table only gave us a count of each of the four groups. Use mutate to create the variable 'visits' which holds the total count as well as 'conRate' which is the conversion rate. 
...
# We can also create the same table as above but with the pages that did not convert and use the variable 'NoconRate' to store this. The conRate from the previous table and the NoconRate from this table should total to 100%. 
...
# In order to create a more in depth graph, group the data by the group (Treatment vs Control) as well as the day. Since our data was collected only in January, there are no overlaps of days in different months. The Data_sum variable now shows the conversion rate of each day of each group. 
...
# Chi-Square Test
# The assumptions of a chi-square test are: 1. It's a large sample (expected frequency of any cell is greater than 5), 2. There's independence of observations. We have a sample size of 290,584 data points with over 30,000 converted users and over 255,000 converted users checking off assumption 1. Each of our collected data points are also independent since any duplicate user id was removed in the cleaning process. Therefore, we can use the chi-square test and now we will state the hypotheses. 
# Our null hypothesis is: There is no significant difference in conversion rates between the old and new page. 
# Our alternative hypothesis is: There is a significant difference in conversion rates between the old and new page.
...
# Since the p-value is 0.1918 > 0.05 (alpha) we fail to reject the null hypothesis. There is no difference in conversion rates between the old and new page. Based on this result, 0 being inside the confidence interval, and from the visualizations we created that showed little to no change between the groups, we would recommend that the company remains with the old page design since the updated page results in no change in conversion rates. Further follow-up experiments should be conducted to see if other changes can improve conversion rates.  
